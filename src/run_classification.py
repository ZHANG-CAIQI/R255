...
def main():
    ...
    fh = logging.FileHandler('log/{}.log'.format(time_value.strftime('%Y_%m_%d_%H_%M_%S')))
    ...
    task = sys.argv[1]
    logger.info(f'running baselines for task {task}')
    ...
    for i in [0,1,2]:
        ...
        for mode, loader in zip(['valid','test','train'], [validation_loader, testing_loader, training_loader]):
            logger.info(f'----------------Evaluating for {mode} set----------------')
            outputs, targets = validation(loader)
            outputs = np.array(outputs) >= 0.5
            logger.info(classification_report(y_true=targets, y_pred=outputs,digits=6))
